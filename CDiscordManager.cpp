...
#include "CDiscordManager.h"
    ...
    #define DISCORD_CLIENT_ID 1426639340741595187
#endif
...
void CDiscordManager::Initialize()
{
    ...
    m_StoredActivity.GetAssets().SetLargeImage("Patriot GTA");            // Always thing
    m_StoredActivity.GetAssets().SetSmallImage("Patriot GTA");
    // m_StoredActivity.GetAssets().SetSmallText("Patriot GTA");
    m_StoredActivity.GetAssets().SetLargeText("Грає у Patriot GTA");
...
void CDiscordManager::setMainMenu()
{
    // m_StoredActivity.GetAssets().SetSmallImage("play");
}
...
void CDiscordManager::UpdateActivity(std::function<void(EDiscordRes)> callback)
{
    ...
    m_DiscordCore->ActivityManager().UpdateActivity(m_StoredActivity,
                                                    [=](discord::Result res)
                                                    {
                                                        callback(static_cast<EDiscordRes>(res));
                                                        if (res != discord::Result::Ok)
                                                            WriteDebugEvent(SString("[DISCORD]: Updating activity failed, code: %i", static_cast<int>(res)));
                                                    });
}
...
void CDiscordManager::updateMaxPlayer()
{
...
void CDiscordManager::RegisterPlay(bool connected)
{
    if (!connected)
    {
        m_StoredActivity.GetAssets().SetSmallText("У головному меню");
        m_StoredActivity.GetAssets().SetSmallImage("avaa");
        //m_StoredActivity.GetSecrets().SetJoin("MTI4NzM0OjFpMmhuZToxMjMxMjM= ");
        //m_StoredActivity.GetParty().SetId("ae488379-351d-4a4f-ad32-2b9b01c91657");
        m_StoredActivity.GetParty().GetSize().SetCurrentSize(0);
        m_StoredActivity.GetParty().GetSize().SetMaxSize(0);
    ...
    std::lock_guard<std::mutex> guardian(m_ThreadSafety);
    m_StoredActivity.SetDetails("Очікуємо ім'я сервера...");
    m_StoredActivity.GetAssets().SetSmallText(SString("Проводить час", *ipaddr, port));
