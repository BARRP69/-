...
#define IP_TO_AUTOCONNECT   "51.75.44.14" // заказ 51.75.44.14:22117
#define PORT_TO_AUTOCONNECT 22117


#define NATIVE_RES_X    1920.0f
#define NATIVE_RES_Y    1080.0f

#define NATIVE_BG_X    1920.0f
#define NATIVE_BG_Y    1080.0f

#define NATIVE_LOGO_X    379.0f
#define NATIVE_LOGO_Y    107.0f
...
// Prefer new asset locations first; engine will fall back to legacy if missing
#define CORE_ASSETS_DIR             "assets\\ui"
#define CORE_LEGACY_DIR             "mta\\cgui\\images"
#define CORE_MAIN_MENU_DIR          "mta\\cgui\\images\\main_menu"

// Route main menu visual assets to main_menu directory
#define CORE_MTA_STATIC_BG          "mta\\cgui\\images\\main_menu\\background.png"
#define CORE_MTA_LOGO               "mta\\cgui\\images\\main_menu\\logo.png"
#define CORE_MTA_FILLER             "mta\\cgui\\images\\main_menu\\filler.png"
#define CORE_MTA_VERSION            "mta\\cgui\\images\\main_menu\\version.png"
// News header stays in the news directory
#define CORE_MTA_LATEST_NEWS        "mta\\cgui\\images\\news\\latest_news.png"
...
// Improve alignment with magical mystery values!
    ...
    // Create our MTA logo
    CVector2D logoSize = CVector2D((NATIVE_LOGO_X * 1.35 / NATIVE_RES_X) * m_iMenuSizeX, (NATIVE_LOGO_Y * 1.35 / NATIVE_RES_Y) * m_iMenuSizeY);
    ...
    m_pLogo->SetPosition(CVector2D(0.5f * m_iMenuSizeX - logoSize.fX / 2, 0.295f * m_iMenuSizeY - logoSize.fY / 2), false);
    ...
    // Force left sidebar layout for all languages
    float fBase = 0.420f; // start Y
    float fGap  = 0.063f; // gap Y
    float xStart = 0.050f; // start X
    // Our disconnect item is shown/hidden dynamically, so we store it seperately
    m_pDisconnect = CreateItem(MENU_ITEM_DISCONNECT, "menu_disconnect.png", CVector2D(xStart, fBase + fGap * 0));
    m_pDisconnect->image->SetVisible(false);

    // Create the menu items positioned per language layout
    // Filepath, Relative position, absolute native size
    // New style button assets: continue/servers/settings/exit
    m_menuItems.push_back(CreateItem(MENU_ITEM_QUICK_CONNECT, "menu_continue.png", CVector2D(xStart, fBase + fGap * 0)));
    m_menuItems.push_back(CreateItem(MENU_ITEM_BROWSE_SERVERS, "menu_servers.png", CVector2D(xStart, fBase + fGap * 1)));
    m_menuItems.push_back(CreateItem(MENU_ITEM_SETTINGS, "menu_settings.png", CVector2D(xStart, fBase + fGap * 2)));
    m_menuItems.push_back(CreateItem(MENU_ITEM_QUIT, "menu_exit.png", CVector2D(xStart, fBase + fGap * 3)));
    ...
    // Align hover/click area with chosen X offset
    m_menuAX = (xStart * m_iMenuSizeX) + m_iXOff;                                                                      // Left side of items
    ...
    // Load the "Новини" header image from mta\\cgui\\images\\news
...
{
  //  if ( GetKeyState(VK_MENU) || GetKeyState(VK_F4) )
  //  {
...
bool CMainMenu::OnDisconnectButtonClick(CGUIElement* pElement)
{
    ...
    // Ask user for confirmation before disconnecting
    m_QuestionBox.Reset();
    m_QuestionBox.SetTitle(_("ВІД'ЄДНАТИСЯ?"));
    m_QuestionBox.SetMessage(_("Ви дійсно бажаєте від'єднатись від сервера?"));
    m_QuestionBox.SetButton(0, _("Ні"));
    m_QuestionBox.SetButton(1, _("Так"));
    m_QuestionBox.SetCallback([](void* /*data*/, uint btn) {
        if (btn == 1)
...
sMenuItem* CMainMenu::CreateItem(unsigned char menuType, const char* szFilename, CVector2D vecRelPosition)
{
    ...
    // Try language-specific path, then new assets folder, then main_menu folder, then legacy folder
    if (g_pCore->GetLocalization()->IsLocalized())
    {
        if (!pImage->LoadFromFile(PathJoin(g_pCore->GetLocalization()->GetLanguageDirectory(), szFilename)))
            if (!pImage->LoadFromFile(PathJoin(CORE_ASSETS_DIR, szFilename)))
                if (!pImage->LoadFromFile(PathJoin(CORE_MAIN_MENU_DIR, szFilename)))
            {
                // Legacy fallbacks for renamed buttons
                const char* alt = szFilename;
                if (strcmp(szFilename, "menu_continue.png") == 0) alt = "menu_quick_connect.png";
                else if (strcmp(szFilename, "menu_exit.png") == 0) alt = "menu_quit.png";
                else if (strcmp(szFilename, "menu_servers.png") == 0) alt = "menu_browse_servers.png";
                else if (strcmp(szFilename, "menu_quick_connect.png") == 0) alt = "menu_continue.png";
                else if (strcmp(szFilename, "menu_quit.png") == 0) alt = "menu_exit.png";
                else if (strcmp(szFilename, "menu_browse_servers.png") == 0) alt = "menu_servers.png";
                if (!pImage->LoadFromFile(PathJoin(CORE_MAIN_MENU_DIR, alt)))
                    if (!pImage->LoadFromFile(PathJoin(CORE_LEGACY_DIR, szFilename)))
                        pImage->LoadFromFile(PathJoin(CORE_LEGACY_DIR, alt));
            }
    }
    else
    {
...
{
    // Localized Ukrainian text as requested
    SStringX strMessage(_("Ви дійсно бажаєте від’єднатись?"));
    CQuestionBox* pQuestionBox = CCore::GetSingleton().GetLocalGUI()->GetMainMenu()->GetQuestionWindow();
    pQuestionBox->Reset();
    pQuestionBox->SetTitle(_("Від’єднатись?"));
    pQuestionBox->SetMessage(strMessage);
    pQuestionBox->SetButton(0, _("Ні"));
    pQuestionBox->SetButton(1, _("Так"));
    pQuestionBox->SetCallback(StaticWantsToDisconnectCallBack, (void*)menuType);
    pQuestionBox->Show();
}

void CMainMenu::AskUserIfHeWantsToQuit()
{
    m_QuestionBox.Reset();
    m_QuestionBox.SetTitle(_("Закрити гру?"));
    m_QuestionBox.SetMessage(_("Ви дійсно бажаєте закрити гру?"));
    m_QuestionBox.SetButton(0, _("Скасувати"));
    m_QuestionBox.SetButton(1, _("Так, закрити гру"));
    m_QuestionBox.SetCallback(StaticWantsToQuitCallBack, nullptr);
    m_QuestionBox.Show();
}

void CMainMenu::StaticWantsToQuitCallBack(void* /*pData*/, uint uiButton)
{
    CLocalGUI::GetSingleton().GetMainMenu()->WantsToQuitCallBack(uiButton);
}

void CMainMenu::WantsToQuitCallBack(uint uiButton)
{
    if (uiButton == 1)
        CCommands::GetSingleton().Execute("exit", "");
}
